...
function assess(record, cfg, industry, source, now) {
  ...
  const callsPending = record.trellus == null;
  const calls = callsPending ? 0.5 : clamp(record.trellus / 100);
  ...
  return { record, estimate, inferred, parts, raw, hits, days, closingPct, expectedSort,
...
function summarize(s, cfg) {
  ...
  if (s.callsPending) {
    out.push(`Call intelligence is not yet scored; the Trellus transcript component holds a neutral ` +
             `${cfg.weights.calls / 2} of ${cfg.weights.calls} until that feed is connected.`);
  }
  return out.join(' ');
}
